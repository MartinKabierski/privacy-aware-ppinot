...
*/
    	//catch corner cases in which all values have same value other than 0. Here we use a best efforts approach to inserting a bit of noise
    	if (max-min == 0) {
    		minMax.setLowerBound(max-(max*factor));
    		minMax.setUpperBound(max+(max*factor));
    		return minMax;
    	}
